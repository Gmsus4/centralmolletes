      ...
      <div className="max-w-6xl mx-auto px-6 sm:px-10 py-16">
        <div className="flex flex-col lg:flex-row gap-10 items-start">
          ...
          <div className="flex-1 flex flex-col gap-6">
            {/* Header */}
            <div className="relative mb-4 p-10 rounded-radius bg-brand-primary overflow-hidden">
              ...
              <div className="relative z-10 flex flex-col gap-4">
                <div className="flex items-center gap-3">
                  <div className="bg-bg-dark rounded-radius w-10 h-10 grid place-items-center shrink-0">
                    <IconLock size={18} className="text-brand-primary" />
                  </div>
                ...
                <p className="text-text-main/70 text-sm leading-relaxed max-w-xl">
            ...
            <div className="flex flex-col gap-4">
              {privacy.map((section, idx) => (
                <div
                  ...
                  className="rounded-radius border-1 border-bg-dark/10 bg-bg-body overflow-hidden scroll-mt-22"
                >
                  ...
                  <ul className="flex flex-col divide-y divide-bg-dark/20 px-7">
                    {section.items.map((item) => (
                      <li key={item} className="flex items-start gap-3 py-4 text-sm text-text-main leading-relaxed">
                        <span className="mt-2 w-1.5 h-1.5 rounded-radius bg-bg-dark shrink-0" />
                        {item}
                      </li>
                    ))}
                  </ul>
          ...
          <aside className="hidden lg:flex flex-col gap-4 w-72 shrink-0 sticky top-24 self-start">
            {/* Índice de secciones */}
            <div className="rounded-radius border-1 border-border-color/10 overflow-hidden">
              ...
              <nav className="flex flex-col divide-y divide-bg-dark/20 bg-bg-body">
                {privacy.map((section, idx) => (
                  <button
                    key={section.title}
                  ...
                  >
                    ...
                    <span className="text-xs text-text-main group-hover:text-text-main transition-colors leading-snug flex-1">{section.title}</span>
            ...
            <div className="rounded-radius bg-brand-primary overflow-hidden p-5 flex flex-col gap-4 border-1 border-border-color/10">
              ...
              <p className="text-text-main/90 text-[11px] leading-relaxed">Puedes ejercer tus derechos TAGB en cualquier momento contactándonos directamente.</p>
            </div>

            {/* CTA contacto */}
            <div className="rounded-radius border-1 border-brand-contrast/10 bg-bg-body p-5 flex flex-col gap-3">
              <div className="flex items-center gap-2">
              ...
              <Link href="/contact" className="w-full text-center rounded-radius bg-brand-contrast text-text-invert font-title text-lg py-1 hover:opacity-90 transition-opacity">
                Contáctanos
              </Link>
              <Link href="/terms" className="w-full text-center rounded-xl border-1 border-brand-contrast text-text-main font-title text-lg py-1 hover:bg-bg-dark/5 transition-colors">
