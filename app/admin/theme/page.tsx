...
import { IconPlus, IconCheck, IconPaint } from "@tabler/icons-react"
            ...
            {themes.map((theme) => (
              <div
                ...
                className={`relative flex items-center justify-between gap-4 px-5 py-4 border transition-colors ${
                  theme.isActive ? "border-stone-900 bg-stone-50" : "border-stone-200 bg-white hover:border-stone-300"
                }`}
              >
                {/* Indicador activo — barra izquierda */}
                {theme.isActive && <span className="absolute left-0 top-0 bottom-0 w-0.5 bg-stone-900" />}
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div className="flex items-center gap-1">
                        {[theme.bgBody, theme.brandPrimary, theme.brandContrast, theme.textMuted, theme.bgDark].map((color, i) => (
                          <span key={i} className="w-3 h-3 rounded-full border border-stone-200" style={{ background: color }} />
                        ))}
                      </div>
