...
function SocialIcon({ platform }: { platform: string }) {
  ...
  return Icon ? <Icon size={12} /> : <span className="text-[10px] font-bold">{platform[0].toUpperCase()}</span>
}

// ─── Contact field ────────────────────────────────────────────────────────────

function ContactField({ icon: Icon, label, value }: { icon: React.ComponentType<{ size?: number }>; label: string; value?: string | null }) {
  return (
    <div className="flex flex-col gap-1">
      ...
      {value ? <span className="text-sm font-medium text-foreground">{value}</span> : <span className="text-sm text-muted-foreground/50 italic">Sin {label.toLowerCase()}</span>}
    </div>
  )
}
    ...
    <LayoutAdminSection namePage="Contacto" maxWidth="max-w-6xl">
      ...
      {contacts.length === 0 ? (
        <EmptyState
          ...
          label="Información de contacto ausente"
          description="No hay datos de contacto registrados. Añade un teléfono o email para que no pierdas ninguna oportunidad."
          actionLabel="Subir datos de contacto"
          actionHref="/admin/contact/new"
          ...
          {contacts.map((contact, i) => (
            <Link href={`/admin/contact/${contact.id}`} key={contact.id} className="flex flex-col gap-5 p-5 border rounded-lg bg-card hover:shadow-sm transition-shadow duration-200">
              ...
              <div className="flex items-center justify-between">
                <span className="text-[10px] uppercase tracking-[0.3em] text-muted-foreground">#{String(i + 1).padStart(2, "0")}</span>
              </div>
              ...
              {contact.socialLinks.length > 0 && (
                <>
                  ...
                  <div className="flex flex-col gap-3">
                    <span className="text-[10px] uppercase tracking-[0.25em] text-muted-foreground">Redes sociales</span>
                    <div className="flex flex-wrap gap-2">
                      {contact.socialLinks.map((social) => (
                        <Badge key={social.id} variant={social.isActive ? "secondary" : "outline"} className={`gap-1.5 ${!social.isActive ? "opacity-50 line-through" : ""}`}>
